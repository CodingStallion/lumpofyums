	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
		 ...
		 Connection con = DriverManager.getConnection(
				 "jdbc:mysql://localhost:3306/lump_of_yums","root","password");
		//Step 4: implement the sql query using prepared statement (https://docs.oracle.com/javase/tutorial/jdbc/basics/prepared.html)
		 PreparedStatement ps = con.prepareStatement("insert into recipe(food_name, prep_time, cooking_time, level, description, ingredients, preparation, uid) values(?,?,?,?,?,?,?,?)");
		...
		writer.println("<h1>" + "You have successfully posted a recipe" + 
		"</h1>");
		writer.close(); 
		} 
		}
